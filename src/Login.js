...
const Login = () => {
  ...
  async function login() {
    ...
    await fetch("https://regimeapi.onrender.com/login?" + data, options)
      .then((res) => res.json())
      .then((data) => {
        console.log(data)
        if (data == null) {
    ...
    const fetchPatient = async (patient_id) => {
      console.log(patient_id)
        try {
            const response = await axios.get(`https://regimeapi.onrender.com/check_patient/${patient_id}`);
    ...
    const fetchFlag = async (patient_id) => {
      console.log(patient_id)
        try {
            const response = await axios.get(`https://regimeapi.onrender.com/get_flags/${patient_id}`);
            const val = response.data
            if(val.flags==(-2)){
  ...
  const responseGoogle = async (response) => {
    try {
      ...
      try {
        await axios.post("https://regimeapi.onrender.com/google-login", callbackData, {
          headers: {
